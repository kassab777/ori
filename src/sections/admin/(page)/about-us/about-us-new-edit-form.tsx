...
import { updateAboutUs } from 'src/actions/admin/page/about-us';
...
type Props = {
  currentItem?: IAboutUsItem;
  currentRole: string[];
};

// ----------------------------------------------------------------------
export default function NewEditForm({ currentItem, currentRole }: Props) {
  const { t: tprivacy } = useTranslate('about-us');
  const { t: tcommon } = useTranslate('common');

  const ACCEPT_UPDATE_ROLE = Boolean(currentRole.includes('update-about-us'));
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      if (ACCEPT_UPDATE_ROLE) {
        await updateAboutUs(`${currentItem?.id}`, data);
