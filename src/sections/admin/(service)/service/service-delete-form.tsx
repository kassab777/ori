...
import { deleteService } from 'src/actions/admin/service/service';
...
export function ServiceDeleteForm({ currentItem, open, onClose }: Props) {
  ...
  const NewSchema = zod.object({
    confirm: zod.string().min(1, { message: tservice('form.confirm.required') }),
  });
    ...
    try {
      if (currentItem) {
        ...
        if (data?.confirm !== CONFIRM_CODE) {
          ...
          await deleteService(currentItem?.id as string);
  ...
  const renderContent = (
    ...
    >
      ...
      <Box>
        ...
        <Field.Text
          name="confirm"
          label={`${tservice('form.confirm.label')} *`}
  ...
  return (
    ...
    >
      <Form methods={methods} onSubmit={onSubmit}>
        <DialogTitle>{tservice(`dialog.delete`)}</DialogTitle>
        ...
        <DialogActions>
          ...
          <LoadingButton type="submit" variant="contained" loading={isSubmitting} color="error">
            {currentItem && tservice(`button.delete`)}
          </LoadingButton>
        </DialogActions>
      </Form>
    </Dialog>
